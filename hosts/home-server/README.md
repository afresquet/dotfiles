...
| `explo-env.age`                 | `KEY=value` env for the Explo container (see §12). Ships with placeholder values that prevent Explo from doing anything useful until you `agenix -e` it. |
...
Depends on §11 (Navidrome) and §13 (slskd) being set up first — Explo logs
...
2. **Fill in the env secret** — the module ships an encrypted placeholder so
   ...
   SLSKD_API_KEY=<slskd-api-key from §13>
...
Soulseek client used as Explo's download backend (§12) and reachable manually
